...
public class MyStorage {
    ...
    static void prod_input(Scanner s) {
        ...
        String productName = s.nextLine(); //0개로 초기화 필요
        int currentIndex=-1;
        ...
        if (currentIndex!=-1){
            ...
            for (int i = 0; i < products.length; i++) {
                System.out.printf("> "+products[i] +"%n");
            }
        }
    ...
    static void prod_remove(Scanner s) {
        ...
        for (int i = 0; i < products.length; i++) {
            if (removeName.equals(products[i])) {
                products[i] = EMPTY;
                counts[i] = 0;
            }
        }
    ...
    public static void prod_amount_add(Scanner s) {
        ...
        // TODO:
            ...
            counts[foundIdx] +=num;  // 제품의 수량을 증가
    ...
    public static void prod_amount_decrease(Scanner s) {
        ...
        String decreaseProd = s.nextLine();
        System.out.println("원하는 출고량을 입력하세요 :");
        String decreaseNum = s.nextLine();

        String validStr = "0123456789.";
        for (char c : decreaseNum.toCharArray()) {
            if (validStr.indexOf(c) == -1) {
                System.out.println("[Error] 수량은 숫자로 입력해야 합니다.");
                return;
            }
        }
        int decNum = Integer.parseInt(decreaseNum);
        for (int i = 0; i < products.length; i++) {

            if (products[i].equals(decreaseProd)) {
                if (counts[i] > decNum) {
                    counts[i] -=decNum;
